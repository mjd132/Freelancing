                        ...
                        <form action="{{ route('proposals.update',$proposal->id) }}" method="POST">
                            ...
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" id="fId" name="fId"
                                       placeholder="Enter your id" value="{{$proposal->freelancer_id}}">
                            </div>
                            <div class="form-group mb-2">
                                <label for="description">Description</label>
                                <textarea class="form-control" id="description" name="description" rows="5"
                                          placeholder="Enter description">{{$proposal->description}}</textarea>
                            </div>
                            <div class="form-group mb-2">
                                <label for="price">Price</label>
                                <input class="form-control w-25" id="price" name="price" type="number"
                                       placeholder="Enter price" value="{{$proposal->price}}"/>
                            </div>
                            <div class="form-group mb-2">
                                <label for="delivery_time">Delivery Time</label>
                                <input class="form-control w-25" id="delivery_time" name="delivery_time" type="date"
                                       value="{{$proposal->delivery_time->toDateString()}}"/>
                            </div>
